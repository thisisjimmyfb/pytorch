...
def _get_seq_nr():
    ...
    if current_meta.get("in_grad_fn", 0) > 0:
        ...
        annotation_log.debug("seq_nr from current_meta grad_fn_seq_nr")
        new_seq_nr = current_meta["grad_fn_seq_nr"][-1]

    elif torch.fx.traceback.has_preserved_node_seq_nr():
        ...
        annotation_log.debug("seq_nr from current_meta seq_nr")
    ...
    if not torch.fx.traceback.has_preserved_node_seq_nr():
        # See Note [Functionalization View Replay Annotation]
        # Overriding some node meta with the original node meta of the
        # regenerated node.
        replay_node: Node = fx_traceback.get_current_replay_node()

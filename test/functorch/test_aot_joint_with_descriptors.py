...
('call_function', 't_9', {'test': 1})""",
    ...
    def test_annotate_invoke_subgraph_simple(self):
        ...
        self.assertEqual(
            top_level_groups,
            [
                ["getitem_1", "invoke_subgraph", "invoke_subgraph_1"],
                ["getitem"],
                ["sub"],
            ],
        )
...
class inner_f(torch.nn.Module):
    ...
    class repeated_subgraph0(torch.nn.Module):
        def forward(self, arg0_1: "f32[4, 3]"):
            # Annotation: {'mod_name': 'bar', 'seq_nr': -1} File: /data/users/shangdiy/pytorch/test/functorch/test_aot_joint_with_descriptors.py:1152 in forward, code: y = x.sin()
            sin: "f32[4, 3]" = torch.ops.aten.sin.default(arg0_1);  arg0_1 = None

            # Annotation: {'mod_name': 'bar', 'seq_nr': 0} File: /data/users/shangdiy/pytorch/test/functorch/test_aot_joint_with_descriptors.py:1153 in forward, code: return y * 1
